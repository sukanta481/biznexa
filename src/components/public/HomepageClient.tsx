    ...
    <>
      <section className="relative min-h-0 md:min-h-[85vh] flex items-center justify-center overflow-hidden px-6 md:px-8 py-24 md:py-0" style={{ background: '#000308' }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto text-center">
          <div className="mb-6 inline-flex items-center gap-2 px-4 py-1.5" style={{ border: '1px solid rgba(0,229,255,0.25)', background: 'rgba(0,229,255,0.05)' }}>
            <span className="material-symbols-outlined text-sm" style={{ color: '#00e5ff', fontVariationSettings: "'FILL' 1" }}>
              bolt
            </span>
            <span className="font-label text-[10px] uppercase tracking-[0.2em]" style={{ color: 'rgba(0,229,255,0.8)' }}>
              {content.hero.badgeText}
            </span>
          </div>

          <h1
            className="font-headline text-4xl md:text-8xl font-bold tracking-tight md:tracking-tighter mb-6 md:mb-8 leading-[1.1] md:leading-[0.9]"
            style={{ color: '#fff', textShadow: '0 0 30px rgba(0,229,255,0.45)' }}
          >
            <span className="md:hidden">
              {content.hero.leadText}{' '}
              <span style={{ color: '#00e5ff', textShadow: '0 0 20px rgba(0,229,255,0.8)' }}>
            ...
            <span className="hidden md:inline">
              {content.hero.leadText}
              <br />
              <span style={{ color: '#00e5ff', textShadow: '0 0 30px rgba(0,229,255,0.8)' }}>
                {content.hero.accentText}
              </span>
            </span>
          </h1>

          <p className="max-w-xs md:max-w-2xl mx-auto text-base md:text-xl font-body leading-relaxed mb-8 md:mb-12" style={{ color: 'rgba(255,255,255,0.55)' }}>
          ...
          <div className="flex flex-col sm:flex-row w-full sm:w-auto items-center justify-center gap-4 sm:gap-6 mt-8 px-6 sm:px-0">
            <Link href={content.hero.primaryCtaHref} className="cyber-cta-btn w-full sm:w-auto py-4 px-10">
              {content.hero.primaryCtaLabel}
            </Link>
            <Link href={content.hero.secondaryCtaHref} className="cyber-cta-btn-ghost w-full sm:w-auto py-4 px-10">
      ...
      <section className="py-12 md:border-y md:border-white/5 md:bg-surface-container-low/30 px-6 md:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
          {content.stats.map((stat) => (
            <GlowCard key={`${stat.value}-${stat.label}`} customSize glowColor="green" className="glass-panel md:!bg-transparent md:!backdrop-blur-none p-6 md:p-0 rounded-2xl md:rounded-none border border-outline-variant/10 md:border-none text-center">
      ...
      <section className="py-24 px-8 max-w-7xl mx-auto" id="services">
        ...
        <div className="flex md:grid overflow-x-auto md:overflow-visible scrollbar-hide snap-x snap-mandatory md:snap-none pb-8 md:pb-0 px-6 md:px-0 gap-6 md:grid-cols-12 -mx-6 md:mx-0">
          ...
          <GlowCard customSize glowColor="purple" className="snap-center shrink-0 w-[85vw] md:w-auto md:col-span-4 group relative overflow-hidden rounded-3xl md:rounded-xl bg-surface-container-high border border-outline-variant/20 md:border-cyber-border p-8 md:p-10 min-h-[350px] md:h-[400px] hover-lift transition-all flex flex-col justify-end md:justify-start">
          ...
          <GlowCard customSize glowColor="orange" className="snap-center shrink-0 w-[85vw] md:w-auto md:col-span-8 group relative overflow-hidden rounded-3xl md:rounded-xl glass-panel cyber-border p-8 md:p-10 min-h-[350px] md:h-[400px] flex flex-col justify-end hover-lift transition-all">
      ...
      <section className="py-20 md:py-24 pt-8 md:pt-24 px-0 md:px-8 max-w-7xl mx-auto bg-surface-container-low/50 md:bg-transparent">
        ...
        <div className="flex md:grid overflow-x-auto md:overflow-visible scrollbar-hide snap-x snap-mandatory md:snap-none pb-8 md:pb-0 px-6 md:px-0 gap-6 md:gap-8 md:grid-cols-3">
          {content.testimonials.map((testimonial) => (
            <GlowCard key={`${testimonial.name}-${testimonial.company}`} customSize glowColor="blue" className="snap-center shrink-0 w-[85vw] md:w-auto p-10 rounded-2xl md:rounded-xl bg-surface-container-high border border-white/5 italic text-on-surface-variant relative hover-lift transition-all">
